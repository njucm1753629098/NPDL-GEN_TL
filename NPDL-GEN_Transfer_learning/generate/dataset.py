...
def sample(model, x, steps, temperature=1.0, sample=False, top_k=None):
    ...
    action_probs = torch.cat([p.unsqueeze(1) for p in action_probs], dim=1)
    action_log_probs = torch.cat([lp.unsqueeze(1) for lp in action_log_probs], dim=1)
       
    return x, action_probs, action_log_probs
